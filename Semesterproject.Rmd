...
kable(head(schreck_locations), caption = "Table 2: Schreck location data")
kable(head(schreck_agenda), caption = "Table 3: Schreck on/off, interval and parameter data")
```
...
```{r, fig.cap= "Figure 1: Wild boar locations (blue) and schreck locations (red) uncropped"}
...
```{r, fig.cap="Figure 9: Static map (before, during, after) of WSS_2016_01, radius = 200m, 10 days before until 10 days after schreck event"}
...
WSS_2016_01 and WSS_2016_05 showed both the expected pattern but the parameter settings are different in terms of mode and volume. Taking in consideration the crop species, we assume, that the standard mode in combination with the higher volume was sufficent to defend the meadow and harvesting crops afford tougher defence strategies. Scare offs in 2015 were all installed on wheat (/potato) crops. The mode of WSS_2015_01 and WSS_2015_03 was switched in the mid of june from standard to agressiv and the volume was enhanced as well. That could explain the decrease of the wildboar occurence in the middle of the scare off events. For WSS_2015_04 no switch of mode was performed, but an increase of volume after the first half of the Schreck event, where also a decrease of wild boar occurence was observed.
