...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
# Create an interactive map with the Wildschwein Locations and the Schreck Location for one specific Schreck

#WSS_2015_01
  # Filter schreck agenda to specific schreck
  specific_schreck15_01 <- schreck_agenda_and_locations_merged %>%
    filter(id == "WSS_2015_01")
  
  # Draw a circle around that schreck
  specific_schreck_circle15_01 <- st_buffer(specific_schreck15_01, dist = 500)
  ...
  specific_schreck_wildschwein15_01 <- wildschwein_BE_sf_cropped %>%
    filter(
      DatetimeUTC > as.Date(specific_schreck15_01$datum_on) - 10,
      DatetimeUTC < as.Date(specific_schreck15_01$datum_off) + 14
    )
  
  # Filter the above-mentioned wildschwein points to ones within the schreck circle
  specific_schreck_wildschwein_cropped15_01 <- specific_schreck_wildschwein15_01 %>%
    st_filter(specific_schreck_circle15_01)
  
  # Create a new convenience variable (date, without time)
  specific_schreck_wildschwein_cropped15_01 <- specific_schreck_wildschwein_cropped15_01 %>%
    mutate(
      date = as.Date(DatetimeUTC)
    )
  
  # Convert back to lon/lat format (WGS83), otherwise leaflet doesn't work
  wildschwein_4326_15_01 <- st_transform(specific_schreck_wildschwein_cropped15_01, crs = 4326)
 
  specific_schreck_4326_15_01 <- st_transform(specific_schreck15_01, crs = 4326)
 
  
  leaflet(data = specific_schreck_4326_15_01) %>%
    addTiles() %>%
    addCircleMarkers() %>%
    addTimeslider(data = wildschwein_4326_15_01,
...
##WSS_2016_01
  # Filter schreck agenda to specific schreck
  specific_schreck16_01 <- schreck_agenda_and_locations_merged %>%
    filter(id == "WSS_2016_01")
  
  # Draw a circle around that schreck
  specific_schreck_circle16_01 <- st_buffer(specific_schreck16_01, dist = 200)
  
  # Filter wildschwein points by date to only ones occuring within     above-mentioned schreck event (10 days before, during and 14 days after)
  specific_schreck_wildschwein16_01 <- wildschwein_BE_sf_cropped %>%
    filter(
      DatetimeUTC > as.Date(specific_schreck16_01$datum_on) - 10,
      DatetimeUTC < as.Date(specific_schreck16_01$datum_off) + 14
    )
  
  # Filter the above-mentioned wildschwein points to ones within the schreck circle
  specific_schreck_wildschwein_cropped16_01 <- specific_schreck_wildschwein16_01 %>%
    st_filter(specific_schreck_circle16_01)
  
  # Create a new convenience variable (date, without time)
  specific_schreck_wildschwein_cropped16_01 <- specific_schreck_wildschwein_cropped16_01 %>%
    mutate(
      date = as.Date(DatetimeUTC)
    )
  
  # Convert back to lon/lat format (WGS83), otherwise leaflet doesn't work
  wildschwein_4326_16_01 <- st_transform(specific_schreck_wildschwein_cropped16_01, crs = 4326)
 
  specific_schreck_4326_16_01 <- st_transform(specific_schreck16_01, crs = 4326)
 
  leaflet(data = specific_schreck_4326_16_01) %>%
    addTiles() %>%
    addCircleMarkers() %>%
    addTimeslider(data = wildschwein_4326_16_01,
                  options = timesliderOptions(
                    position = "topright",
                    timeAttribute = "DatetimeUTC",
                    sameDate = TRUE,
